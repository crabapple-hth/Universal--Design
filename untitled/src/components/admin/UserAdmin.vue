...
import {apiUserDetailTotal, apiUserList, apiUserSave} from "@/net/index.js";
...
function saveUserDetail() {
  ...
  apiUserSave(editor.temp, () => {
    const user = userTable.data.find(user => user.userid === editor.id)
...
<template>
  <div class="user-admin">
    ...
    <div class="desc">
      在这里管理论坛所有用户,包括用户信息，封禁和禁言处理
    </div>
    <el-table :data="userTable.data" style="width: 100%" height="420">
      ...
      <el-table-column label="角色" width="100" align="center">
        <template #default="{row}">
          ...
          <el-tag  v-if="row.role === 'user'">普通用户</el-tag>
        </template>
      </el-table-column>
    ...
    <el-drawer v-model="editor.display">
      ...
      <el-form label-position="top">
        ...
        <div style="margin-top: 10px;color: #606266;font-size: 14px">
          注册时间: {{ new Date(editor.temp.creatTime).toLocaleString() }}
        </div>
        <el-divider/>
      </el-form>
      <template #footer>
